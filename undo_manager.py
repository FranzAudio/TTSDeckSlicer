...
class UndoManager:
    def __init__(
        ...
        self._backup_file = os.fspath(
            backup_file or os.path.join(os.path.expanduser("~"), ".ttsdeck_backup.json")
        )
        ...
        try:
            if os.path.exists(self._backup_file):
                with open(self._backup_file, "r") as f:
                    backup = json.load(f)
                    state = {tuple(map(int, k.split(","))): v for k, v in backup["state"].items()}
                    return state
        except Exception:
            pass
        return None
