...
class CardSearchDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        self.include_encounter_checkbox.setToolTip(
            "Include encounter/enemy cards in search results"
        )
    ...
    def _load_card_image(self, card_data):
        ...
        try:
            ...
            LOGGER.warning("Error loading card image: %s", exc)
    ...
    def _on_cards_loaded(self, success):
        ...
        if success:
            ...
            self.results_info.setText(
                "Failed to load card database. Please check your internet connection."
            )
    ...
    def _perform_search(self, text: str):
        ...
        if count > 0:
            ...
            if len(text) >= 2:  # Only show manual option if user has typed something
                self.results_info.setText(
                    f"No matching cards found. You can use '{text}' as a manual name."
                )
