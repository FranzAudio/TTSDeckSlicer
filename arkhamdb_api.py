...
class ArkhamDBAPI:
    ...
    def get_card(self, code: str) -> Optional[dict]:
        ...
        for endpoint in endpoints:
            self._rate_limit()
            try:
                headers = {"User-Agent": "TTSDeckSlicer/1.4", "Accept": "application/json"}
                ...
                if response.status_code == 200:
                    ...
                    return card
                ArkhamDBAPI._logger.warning(
                    "ArkhamDB returned HTTP %s for %s",
                    response.status_code,
                    endpoint,
                )
            except requests.RequestException as exc:
                ArkhamDBAPI._logger.warning("ArkhamDB card request failed for %s: %s", code, exc)
    ...
    def _load_all_cards(self) -> bool:
        ...
        try:
            ...
            response = self._session.get(
                f"{self.BASE_URL}/cards?encounter=1", headers=headers, timeout=10
            )
            if response.status_code == 200:
                all_cards = response.json()
                if isinstance(all_cards, list) and len(all_cards) > 0:
                    self._set_shared_data(all_cards)
                    ArkhamDBAPI._logger.info("Loaded %d cards from ArkhamDB", len(all_cards))
                    return True
        except (requests.RequestException, ValueError) as exc:
            ArkhamDBAPI._logger.warning("Failed to load all cards: %s", exc)
        ...
        if cards:
            ...
            ArkhamDBAPI._logger.info("Loaded %d cards from ArkhamDB", len(cards))
            return True
    ...
    def search_cards(
        self, query: str, limit: Optional[int] = None, include_encounter: bool = True
    ) -> List[dict]:
    ...
    def get_card_name_suggestions(
        self, partial_name: str, limit: int = 200, include_encounter: bool = True
    ) -> List[dict]:
        """
    ...
    def _is_encounter_card(self, card: dict) -> bool:
        ...
        return (
            (faction in encounter_factions and type_code in encounter_types)
            or card.get("encounter_code") is not None
            or card.get("encounter_position") is not None
        )
